// Copyright 2021-present StarRocks, Inc. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#pragma once

#include <cstdint>
#include <string>
#include <vector>

#include "distribution/dst_distribution_store.h"
#include "distribution/scaling.h"
#include "utils/decimal.h"
#include "utils/row_streams.h"

namespace benchgen::tpcds::internal {

struct PromotionRowData {
  int64_t promo_sk = 0;
  std::string promo_id;
  int32_t start_date_id = 0;
  int32_t end_date_id = 0;
  int64_t item_sk = 0;
  Decimal cost;
  int32_t response_target = 0;
  std::string promo_name;
  bool channel_dmail = false;
  bool channel_email = false;
  bool channel_catalog = false;
  bool channel_tv = false;
  bool channel_radio = false;
  bool channel_press = false;
  bool channel_event = false;
  bool channel_demo = false;
  std::string channel_details;
  std::string purpose;
  bool discount_active = false;
  int64_t null_bitmap = 0;
};

class PromotionRowGenerator {
 public:
  PromotionRowGenerator(double scale);

  void SkipRows(int64_t start_row);
  PromotionRowData GenerateRow(int64_t row_number);
  void ConsumeRemainingSeedsForRow();

 private:
  static std::vector<int> ColumnIds();

  Scaling scaling_;
  DstDistributionStore distribution_store_;
  RowStreams streams_;
  int32_t start_date_base_ = 0;
  Decimal cost_;
};

}  // namespace benchgen::tpcds::internal
