...
struct CatalogReturnsRowData {
  int32_t returned_date_sk = 0;
  int32_t returned_time_sk = 0;
  int64_t item_sk = 0;
  int64_t refunded_customer_sk = 0;
  int64_t refunded_cdemo_sk = 0;
  int64_t refunded_hdemo_sk = 0;
  int64_t refunded_addr_sk = 0;
  int64_t returning_customer_sk = 0;
  int64_t returning_cdemo_sk = 0;
  int64_t returning_hdemo_sk = 0;
  int64_t returning_addr_sk = 0;
...
class CatalogReturnsRowGenerator {
 public:
  CatalogReturnsRowGenerator(double scale);

  void SkipRows(int64_t start_row);
  CatalogReturnsRowData GenerateRow(int64_t row_number);
  void ConsumeRemainingSeedsForRow();

 private:
  static std::vector<int> ColumnIds();
  CatalogReturnsRowData BuildReturnRow(const CatalogSalesRowData& sale);
  void LoadNextReturns();

  Scaling scaling_;
  DstDistributionStore distribution_store_;
  RowStreams streams_;
  CatalogSalesRowGenerator sales_generator_;
  int64_t current_order_ = 0;
  std::vector<CatalogReturnsRowData> pending_returns_;
