...
arrow::Status SupplierRowGenerator::Init() {
  ...
  if (seed_mode_ == DbgenSeedMode::kAllTables) {
    status =
        AdvanceSeedsForTable(&random_state_, TableId::kSupplier, scale_factor_);
    if (!status.ok()) {
      return status;
    }
  }
...
void SupplierRowGenerator::GenerateRow(int64_t row_number, supplier_t* out) {
  if (!out) {
    return;
  }
  ...
  std::snprintf(out->name, sizeof(out->name), kSNameFmt, kSNameTag,
                static_cast<long>(row_number));
  std::string address;
  out->alen = VariableString(kSAddrLen, kSAddrSd, &random_state_, &address);
  ...
  random_state_.RowStop(DbgenTable::kSupp);
}
